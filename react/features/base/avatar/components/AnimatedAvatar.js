...
import { getParticipantById, getParticipantByIdOrUndefined } from '../../participants';
import { connect } from '../../redux';
import { getLocalAudioTrack, getTrackByMediaTypeAndParticipant } from '../../tracks';
import { AVATAR_SETTINGS, getAvatarColor, getAvatarSettingName, getInitials, isCORSAvatarURL } from '../functions';

import { AnimatedStatelessAvatar } from './';
...
class Avatar<P: Props> extends PureComponent<P, State> {
    ...
    componentDidUpdate(prevProps: P) {
        ...
        console.log('[tinkerday] componentDidUpdate state:', this.state);
    ...
    componentWillUnmount() {
        if (this.props._audioTrack) {
            console.log('[tinkerday] componentWillUnmount');
            const { jitsiTrack } = this.props._audioTrack;

            jitsiTrack && jitsiTrack.off(JitsiTrackEvents.TRACK_AUDIO_LEVEL_CHANGED, this.updateAudioLevel);
        }
    }
    ...
    updateAudioLevel(level) {
        if (this.props._audioTrack) {
            console.log('[tinkerday] updateAudioLevel', level);
            this.setState({
                audioLevel: level
            });
        }
     ...
     */
    render() {
        console.log('[tinkerday] render AnimatedAvatar');
...
export function _mapStateToProps(state: Object, ownProps: Props) {
    ...
    console.log('[tinkerday] mapStateToProps', state, ownProps);

    const { colorBase, displayName, participantId } = ownProps;
    const _participant: ?Object = participantId && getParticipantById(state, participantId);
    const participantOrUndefined = getParticipantByIdOrUndefined(state, participantId);
    const isLocal = participantOrUndefined?.local ?? true;
